...
{
    /// <summary>
    /// 服务端 IO线程同步调用 不返回数据（不应答客户端） 示例接口
    /// </summary>
    public interface ISendOnlyController
    {
        ...
        CommandServerSendOnly SendOnly(CommandServerSocket socket, int parameter1, int parameter2);
        /// <summary>
        /// 不返回数据（不应答客户端）
        /// </summary>
        /// <param name="parameter">参数</param>
        /// <returns>返回值类型必须为 AutoCSer.Net.CommandServerSendOnly</returns>
        CommandServerSendOnly SendOnly(int parameter);
    }
    /// <summary>
    /// 服务端 IO线程同步调用 不返回数据（不应答客户端） 示例接口实例
    /// </summary>
    internal sealed class SendOnlyController : ISendOnlyController
    {
        ...
        /// <param name="parameter2">参数</param>
        /// <returns>返回值类型必须为 AutoCSer.Net.CommandServerSendOnly</returns>
        CommandServerSendOnly ISendOnlyController.SendOnly(CommandServerSocket socket, int parameter1, int parameter2)
        {
        ...
        /// <param name="parameter">参数</param>
        /// <returns>返回值类型必须为 AutoCSer.Net.CommandServerSendOnly</returns>
        CommandServerSendOnly ISendOnlyController.SendOnly(int parameter)
        {
