...
namespace AutoCSer.Example.CommandServer.Server.AsyncTaskQueue
{
    ...
    public interface ISendOnlyController
    {
        ...
        Task<CommandServerSendOnly> SendOnly(CommandServerSocket socket, CommandServerCallTaskQueue<int> queue, int parameter1, int parameter2);
        /// <summary>
        /// 不返回数据（不应答客户端）
        /// </summary>
        /// <param name="queue">当前执行队列上下文，必须定义在所有数据参数之前</param>
        ...
        /// <returns>返回值类型必须为 AutoCSer.Net.CommandServerSendOnly</returns>
        Task<CommandServerSendOnly> SendOnly(CommandServerCallTaskLowPriorityQueue<int> queue, int parameter);
    }
    /// <summary>
    ...
    internal sealed class SendOnlyController : ISendOnlyController
    {
        ...
        Task<CommandServerSendOnly> ISendOnlyController.SendOnly(CommandServerSocket socket, CommandServerCallTaskQueue<int> queue, int parameter1, int parameter2)
        {
        ...
        /// <param name="parameter">参数</param>
        /// <returns>必须是 async Task</returns>
        Task<CommandServerSendOnly> ISendOnlyController.SendOnly(CommandServerCallTaskLowPriorityQueue<int> queue, int parameter)
        {
