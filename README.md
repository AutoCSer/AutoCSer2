...
支持面向对象编程的特性使它在**内存充足的场景**中几乎无所不能，当然前提是开发者至少需要可以**灵活运用各种常用数据结构与算法**（其实绝大多数问题都可以求助 AI 解决）。比如你可以编写各种常用数据结构就可以封装出逻辑功能远超 Redis API 的组件，你还可以提供向量 API、图 API、时序聚合 API 甚至关系型数据库 API（非 SQL），你的开发能力决定它的功能上限，只需要你能够在**内存中实现这些数据结构与算法**。  
...
NuGet 发布了核心程序集 AutoCSer.dll 的 3 个版本包括 [.NET8](https://www.nuget.org/packages/AutoCSer.NET8/) | [.NET Standard 2.1](https://www.nuget.org/packages/AutoCSer2.1/) | [.NET Standard 2.0](https://www.nuget.org/packages/AutoCSer2/)，.NET Framework 4.5 版本以及其它项目需要到 [github](https://github.com/AutoCSer/AutoCSer2) 或者 [atomgit](https://atomgit.com/autocser/AutoCSer) 下载源代码自行编译。
# 开箱即用的 RPC
## 定义服务接口
``` csharp
    /// <summary>
