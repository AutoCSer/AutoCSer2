...
namespace AutoCSer.Net
{
    ...
    public abstract class CommandServerController
    {
        ...
        internal void DoCommandOnly(CommandServerSocket socket, ref SubArray<byte> data)
        {
            var exception = default(Exception);
            CommandClientReturnTypeEnum returnType = CommandClientReturnTypeEnum.Unknown;
            try
            {
                returnType = DoCommand(socket, ref data);
            }
            ...
            finally
            {
                ...
                {
                    switch (socket.Method.MethodType)
                    {
...
#endif
                            socket.CancelKeepCallback(returnType, exception);
                            break;
                        case ServerMethodTypeEnum.Task:
                            socket.SendTask(returnType, exception);
                            break;
                        default:
                            socket.Send(returnType, exception);
                            break;
                    }
                }
            }
        ...
        internal void DoCommandOfflineCount(CommandServerSocket socket, ref SubArray<byte> data)
        {
            CommandClientReturnTypeEnum returnType = CommandClientReturnTypeEnum.Unknown;
            var exception = default(Exception);
            try
            ...
            {
                OfflineCount offlineCount = socket.OfflineCount;
                if (returnType == CommandClientReturnTypeEnum.Success)
                {
