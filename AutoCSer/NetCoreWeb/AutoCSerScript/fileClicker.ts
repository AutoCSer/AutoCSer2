...
module AutoCSer {
    export class FileClicker implements IDeclare {
        ...
        private Move(Event: BrowserEvent) {
            HtmlElement.$Id(this.FileId).Style('left', (Event.MouseX - 80) + 'px').Style('top', (Event.MouseY - 8) + 'px').Display(1);
        }
        //private NewInput() {
    ...
    Declare.Create(FileClicker, 'FileClicker', 'mouseover', DeclareType.ParameterId);
}