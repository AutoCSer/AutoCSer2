...
module AutoCSer {
    ...
    export class OverDiv {
        ...
        constructor(Parameter = null) {
            ...
            this.Id = '_' + Pub.GetIdentity() + '_OVER_';
            this.Ids = [];
        }
        Show(Id: string = null, ZIndex = 0) {
            ...
            else HtmlElement.$Create('div').Style('zIndex', OverDiv.OverZIndex).Style('position', 'fixed').Styles('top,left', '0px').Styles('width,height', '100%').Style('backgroundColor', this.Color).Opacity(this.Opacity).Set('id', this.Id).To();
        }
        Hide(Id: string = null) {
            if (Id) {
                if (this.Ids.length) {
