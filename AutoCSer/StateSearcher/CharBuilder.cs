...
namespace AutoCSer.StateSearcher
{
    ...
    internal unsafe struct CharBuilder
    {
                ...
                fixed (char* charFixed = chars.Array)
                {
                    ...
                    *(ushort*)(charStart + sizeof(ushort)) = (ushort)charCount;//特殊字符数量[ushort]
                    ...
                    {//特殊字符二分查找表[charCount*char]
                        ...
                        AutoCSer.Common.CopyTo(start, charStart, charSize);
                        charEnd += charSize;
                    }
            ...
            else if (names.Length != 0)
            {
                ...
                if (name.Length <= 128)
                {
                    ...
                    *(int*)(state + sizeof(int) * 2) = names[0].Value;//名称索引
                    ...
                    AutoCSer.Common.CopyTo(name, prefix);
                ...
                else
                {
                    ...
                    *(int*)(state + sizeof(int)) = 0;//状态位置
                    *(int*)(state + sizeof(int) * 2) = names[0].Value;//名称索引
                    AutoCSer.Common.CopyTo(name, state + *(int*)state);
                }
            }
                    ...
                    if (count == 1)
                    {
                        ...
                        else
                        {
                            *(int*)state = (int)(this.prefix - state);
                            ...
                            {
                                prefixSize <<= 1;
                                AutoCSer.Common.CopyTo(charFixed + current + 1, this.prefix, prefixSize);
        ...
        /// <returns></returns>
#if NetStandard21
        internal static bool Check(KeyValue<string, int>[] values, out string? errorValue)
#else
        internal static bool Check(KeyValue<string, int>[] values, out string errorValue)
#endif
        {
        ...
        internal static Pointer Create(LeftArray<string> names, bool isStaticUnmanaged)
        {
            int index = 0;
            ...
            if (Check(strings, out errorValue)) return new CharBuilder(strings, isStaticUnmanaged).Data;
