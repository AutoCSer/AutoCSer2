...
namespace AutoCSer.Search.WordCountIndexs
{
    ...
    public class QueueContext<T> : StaticSearcherQueueContext<T, WordCounter<T>>
        where T : IEquatable<T>
    {
        ...
        internal bool GetWordResults(int version, out LeftArray<KeyValue<SubString, WordCounter<T>>> results)
        {
            if (version == this.version)
            {
                ...
                if (wordResults.Count != 0)
                {
                    ...
                    foreach (KeyValue<HashSubString, WordCounter<T>> keys in wordResults.KeyValues) wordResultArray.Add(new KeyValue<SubString, WordCounter<T>>(keys.Key.String, keys.Value));
                    if (wordResultArray.Length > 1) wordResultArray.Sort(wordResultSortHandle);
                }
                results = wordResultArray;
                return true;
            }
            results = new LeftArray<KeyValue<SubString, WordCounter<T>>>(0);
            return false;
        }
        ...
        /// <returns></returns>
        private static int wordResultSort(KeyValue<SubString, WordCounter<T>> left, KeyValue<SubString, WordCounter<T>> right)
        {
            int value = left.Value.ResultCount - right.Value.ResultCount;
        ...
        /// 分词搜索结果按结果数量排序
        /// </summary>
        private static readonly Func<KeyValue<SubString, WordCounter<T>>, KeyValue<SubString, WordCounter<T>>, int> wordResultSortHandle = wordResultSort;
    }
}
