...
module AutoCSer {
    ...
    export class HtmlEditor extends HtmlEditorParameter implements IDeclare {
        ...
        constructor(Parameter: HtmlEditorParameter) {
            super();
            Pub.GetParameterEvents(this, HtmlEditor.DefaultParameter, HtmlEditor.DefaultEvents, Parameter);
        ...
        private OverButton(Event: BrowserEvent) {
            if (this.ButtonOverClassName) Event.Element.className = this.ButtonOverClassName;
        }
        private OutButton(Event: BrowserEvent) {
            if (this.ButtonOutClassName) Event.Element.className = this.ButtonOutClassName;
        }
        ...
        private OnUploadImage(Value: IHttpRequestReturn) {
            var Images = Value.Result as string[];
            if (Images) {
        ...
        private FormatCode() {
            ...
            for (var Spans = new HtmlElement("/span", Body).GetElements(), Index = Spans.length; Index; View.DeleteMark(Spans[--Index]));
            this.SetMode(OldMode);
        }
        private ChangeParagraph() {
            var Paragraph = HtmlElement.$IdElement(this.GetId('paragraph')) as HTMLSelectElement, Value = Paragraph.options[Paragraph.selectedIndex].value;
        ...
        private ChangeFontSize() {
            ...
            (frames[this.EditorFrameId] as Window).document.execCommand('fontsize', false, HtmlEditor.FontSizes[FontSize.selectedIndex - 1]);
    ...
    Declare.Create(HtmlEditor, 'HtmlEditor', 'click', DeclareType.EventElement);
