...
namespace AutoCSer.Net.Packet
{
    ...
    public struct Ip6
    {
        ...
        {
            get { return (((uint)data.Array[data.Start] & 15) << 4) + ((uint)data.Array[data.Start + 1] >> 4); }
        }
        /// <summary>
        /// 标识唯一的流标签(实验阶段的可选字段)
        /// </summary>
        public uint FlowTag
        {
            get
            {
                return (((uint)data.Array[data.Start + 1] & 15) << 16) + ((uint)data.Array[data.Start + 2] << 8)
                    + data.Array[data.Start + 3];
            }
        }
        /// <summary>
        ...
        {
            get { return (IpProtocolEnum)data.Array[data.Start + 6]; }
        }
        /// <summary>
        /// 跳数限制
        /// </summary>
        public byte Hops
        {
            get { return data.Array[data.Start + 7]; }
        }
        /// <summary>
        /// 源IP地址
        /// </summary>
        public SubArray<byte> Source
        {
            get
            {
                return new SubArray<byte>(data.Start + 8, 16, data.Array);
            }
        }
        /// <summary>
        /// 目的IP地址
        ...
        public SubArray<byte> Destination
        {
            get
            {
                return new SubArray<byte>(data.Start + 24, 16, data.Array);
            }
        }
        /// <summary>
        ...
        public SubArray<byte> Packet
        {
            get
            {
                return new SubArray<byte>(data.Start + (int)headerEndIndex, data.Length - (int)headerEndIndex, data.Array);
            }
        }
        /// <summary>
        /// IPv6包
                        ...
                        if (protocol < 64)
                        {
                            ...
                            if (packetSize >= 8)
                            {
                                do
                                {
                                    ...
                                    headerEndIndex += 8;
                                    if (protocol == (byte)IpProtocolEnum.IPv6FragmentHeader)
                                    {
