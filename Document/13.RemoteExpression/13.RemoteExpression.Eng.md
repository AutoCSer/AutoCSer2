...
```
**Local variables** or **members of local variables** used in Lambda expressions are treated as constant nodes. These data types **must support serialization** operations. For example, both constantParameter.Value and parameter in the following code are constant nodes.
``` csharp
