...
```
From the above example, it can be seen that the return value of the client API **carries network interaction status information**. Many developers are not accustomed to this type of API return value. AutoCSer provides a client API encapsulation type with the **suffix ReturnValueNode** for all built-in nodes, which directly returns the required data type. These apis will **throw an Exception** when the call status is abnormal. The [client sample code](https://github.com/AutoCSer/AutoCSer2/blob/main/Document/07.MemoryDatabaseNode/Client/DictionaryNode.cs) is as follows:
``` csharp
    /// <summary>
    /// Example of a generic dictionary client node
    /// </summary>
