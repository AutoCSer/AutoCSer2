...
namespace AutoCSer.Document.ServiceThreadStrategy.Server.TaskQueue
{
    ...
    public interface ISynchronousController
    {
        ...
        Task<int> Add(AutoCSer.Net.CommandServerCallTaskQueue<int> queue, int left, int right);
    }
    /// <summary>
    ...
    internal sealed class SynchronousController : ISynchronousController
    {
        ...
        /// <param name="right"></param>
        /// <returns>返回值类型必须为 Task 或者 Task{T}</returns>
        Task<int> ISynchronousController.Add(AutoCSer.Net.CommandServerCallTaskQueue<int> queue, int left, int right)
        {
