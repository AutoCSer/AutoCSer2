...
由于本节内容繁多，初学者可以会觉得部分内容难以理解，所以推荐初学者仅关注标记了**（必要）**的基础章节内容，其它章节可以在掌握了必要的基础章节内容以后做为进阶知识逐步熟悉。
# 应答方式**（必要）**
AutoCSer RPC 是基于 TCP 长连接实现的全双工 RPC 组件，所以 API 支持 3 种基础应答方式。
## 1. 一次性响应**（必要）**
...
## 4. 二阶段响应
状态有关的服务端推送需求，可能需要先给客户端推送一份初始数据，然后在初始数据的基础上推送数据变更日志记录。  
虽然组合一次性响应与持续响应两种 API 可以实现该需求，但是要正确的实现该功能可能需要手动处理一些复杂的状态处理逻辑，比如先获取初始数据然后再触发持续响应可能会造成期间的数据变更日志记录的丢失。  
二阶段响应通过**一个 API 一次性封装两个回调委托**，可以简化服务端与客户端的状态处理逻辑，从网络数据交互层面保证客户端数据的完整性与数据状态的正确性。
## 请求与响应
...
AutoCSer RPC 的服务端共支持 7 种[线程调度策略](https://github.com/AutoCSer/AutoCSer2/blob/main/AutoCSer/Net/CommandServer/Metadata/ServerMethodTypeEnum.cs)，所以服务端 API 定义有 7 大类型，每种线程调度类型都支持 4 种应答方式，同一种应答方式又可能存在多种 API 定义方式。
## 1. Task API**（必要）**
