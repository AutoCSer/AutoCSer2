...
{
    /// <summary>
    /// The server-side synchronous queue thread one-time response API sample interface
    /// 服务端 同步队列线程 一次性响应 API 示例接口
    /// </summary>
    ...
    /// <summary>
    /// The server-side synchronous queue thread one-time response API sample controller
    /// 服务端 同步队列线程 一次性响应 API 示例控制器
    /// </summary>
    internal sealed class SynchronousController: ISynchronousController
    {
        ...
        /// <param name="right"></param>
        /// <returns></returns>
        int ISynchronousController.Add(AutoCSer.Net.CommandServerCallQueue queue, int left, int right)
        {
            return left + right;
        }
    }
