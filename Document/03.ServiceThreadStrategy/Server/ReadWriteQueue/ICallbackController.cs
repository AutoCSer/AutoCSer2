...
{
    /// <summary>
    /// Server read/write queue one-time response callback delegate API sample interface
    /// 服务端 读写队列 一次性响应 回调委托 API 示例接口
    /// </summary>
    public interface ICallbackController
    {
        ...
        void Add(AutoCSer.Net.CommandServerCallReadQueue queue, int left, int right, AutoCSer.Net.CommandServerCallback<int> callback);
    }
    /// <summary>
    /// The server-side read and write queue one-time response callback delegate API sample controller
    /// 服务端 读写队列 一次性响应 回调委托 API 示例控制器
    /// </summary>
