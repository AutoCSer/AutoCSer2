...
{
    /// <summary>
    /// The server-side read and write queue two-stage response API sample interface
    /// 服务端 读写队列 二阶段响应 API 示例接口
    /// </summary>
    public interface ITwoStage‌CallbackController
    {
        ...
        void Callback(AutoCSer.Net.CommandServerCallReadQueue queue, int left, int count, AutoCSer.Net.CommandServerCallback<TwoStageCallbackParameter> callback, AutoCSer.Net.CommandServerKeepCallback<int> keepCallback);
        /// <summary>
        ...
        void CallbackCount(AutoCSer.Net.CommandServerCallWriteQueue queue, int left, int count, AutoCSer.Net.CommandServerCallback<TwoStageCallbackParameter> callback, AutoCSer.Net.CommandServerKeepCallbackCount<int> keepCallback);
    }
    /// <summary>
    /// The server-side read and write queue two-stage response API sample controller
    /// 服务端 读写队列 二阶段响应 API 示例控制器
    /// </summary>
    internal sealed class TwoStage‌CallbackController : ITwoStage‌CallbackController
    {
        ...
        /// 第二阶段持续响应的回调委托包装，最后一个参数类型必须是 AutoCSer.Net.CommandServerKeepCallbackCount{T}</param>
        /// <returns>The return value type must be void</returns>
        void ITwoStage‌CallbackController.CallbackCount(AutoCSer.Net.CommandServerCallWriteQueue queue, int left, int count, AutoCSer.Net.CommandServerCallback<TwoStageCallbackParameter> callback, AutoCSer.Net.CommandServerKeepCallbackCount<int> keepCallback)
        {
