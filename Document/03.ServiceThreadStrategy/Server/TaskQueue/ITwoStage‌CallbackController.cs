...
{
    /// <summary>
    /// The server-side Task asynchronous read and write queue two-stage response API sample interface
    /// 服务端 Task 异步读写队列 二阶段响应 API 示例接口
    /// </summary>
    public interface ITwoStage‌CallbackController
    {
        ...
        System.Threading.Tasks.Task Callback(AutoCSer.Net.CommandServerCallTaskQueue<int> queue, int left, int count, AutoCSer.Net.CommandServerCallback<TwoStageCallbackParameter> callback, AutoCSer.Net.CommandServerKeepCallback<int> keepCallback);
        /// <summary>
        ...
        System.Threading.Tasks.Task CallbackCount(AutoCSer.Net.CommandServerCallTaskQueue<int> queue, int left, int count, AutoCSer.Net.CommandServerCallback<TwoStageCallbackParameter> callback, AutoCSer.Net.CommandServerKeepCallbackCount<int> keepCallback);
    }
    /// <summary>
    /// The server-side Task asynchronous read and write queue two-stage response API sample controller
    /// 服务端 Task 异步读写队列 二阶段响应 API 示例控制器
    /// </summary>
    internal sealed class TwoStage‌CallbackController : ITwoStage‌CallbackController
    {
        ...
        /// 第二阶段持续响应的回调委托包装，最后一个参数类型必须是 AutoCSer.Net.CommandServerKeepCallbackCount{T}</param>
        /// <returns>The return value type must be System.Threading.Tasks.Task</returns>
        System.Threading.Tasks.Task ITwoStage‌CallbackController.CallbackCount(AutoCSer.Net.CommandServerCallTaskQueue<int> queue, int left, int count, AutoCSer.Net.CommandServerCallback<TwoStageCallbackParameter> callback, AutoCSer.Net.CommandServerKeepCallbackCount<int> keepCallback)
        {
            return Task.TwoStage‌CallbackController.CallbackCount(left, count, callback, keepCallback);
        }
    }
}
