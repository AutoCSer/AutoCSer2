...
{
    /// <summary>
    /// The server side supports a synchronous queue for parallel reading and a one-time response callback delegate API sample interface
    /// 服务端 支持并行读的同步队列 一次性响应 回调委托 API 示例接口
    /// </summary>
    public interface ICallbackController
    {
        ...
        void Add(AutoCSer.Net.CommandServerCallConcurrencyReadQueue queue, int left, int right, AutoCSer.Net.CommandServerCallback<int> callback);
    }
    /// <summary>
    /// The server side supports a synchronous queue for parallel reading and a one-time response callback delegate API sample controller
    /// 服务端 支持并行读的同步队列 一次性响应 回调委托 API 示例控制器
    /// </summary>
