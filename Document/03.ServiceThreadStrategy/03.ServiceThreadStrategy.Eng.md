...
The server of AutoCSer RPC supports a total of 7 [thread scheduling strategies](https://github.com/AutoCSer/AutoCSer2/blob/main/AutoCSer/Net/CommandServer/Metadata/ServerMethodTypeEnum.cs). Therefore, there are 7 major types of server API definitions. Each thread scheduling type supports 3 response methods, and the same response method may have multiple API definition methods.
## 1. Task API
...
```
In addition to the [synchronous pesponse API](https://github.com/AutoCSer/AutoCSer2/blob/main/Document/03.ServiceThreadStrategy/Server/Task/ISynchronousController.cs), the [callback delegate response API](https://github.com/AutoCSer/AutoCSer2/blob/main/Document/03.ServiceThreadStrategy/Server/Task/IKeepCallbackController.cs) is also supported, allowing the server to respond to client requests through delegate callbacks at appropriate times. It is mainly used for event-triggered callback response requirements.  
