    ...
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            await using (AutoCSer.Net.CommandListener commandListener = new AutoCSer.Net.CommandListenerBuilder(32)
                ...
                .Append<Server.Task.ITwoStageCallbackController>(new Server.Task.TwoStageCallbackController())
                ...
                .Append<Server.TaskQueue.ITwoStageCallbackController>(new Server.TaskQueue.TwoStageCallbackController())
                ...
                .Append<Server.Queue.ITwoStageCallbackController>(new Server.Queue.TwoStageCallbackController())
                ...
                .Append<Server.ConcurrencyReadQueue.ITwoStageCallbackController>(new Server.ConcurrencyReadQueue.TwoStageCallbackController())
                ...
                .Append<Server.ReadWriteQueue.ITwoStageCallbackController>(new Server.ReadWriteQueue.TwoStageCallbackController())
                ...
                .Append<Server.Synchronous.ITwoStageCallbackController>(new Server.Synchronous.TwoStageCallbackController())
        ...
        private static async Task<bool> client()
        {
            try
            {
                ...
                TwoStageCallback callback = new TwoStageCallback();
                using (var commandKeepCallback = await client.ServerTask_TwoStageCallbackController.Callback(callback.Parameter.Start, callback.Parameter.Count, callback.Callback, callback.KeepCallback))
                {
                ...
                callback = new TwoStageCallback();
                using (var commandKeepCallback = await client.ServerTaskQueue_TwoStageCallbackController.Callback(-1, callback.Parameter.Start, callback.Parameter.Count, callback.Callback, callback.KeepCallback))
                {
                    if (!await callback.Wait(commandKeepCallback))
                    {
                        return AutoCSer.Breakpoint.ReturnFalse();
                ...
                callback = new TwoStageCallback();
                using (var commandKeepCallback = await client.ServerQueue_TwoStageCallbackController.Callback(callback.Parameter.Start, callback.Parameter.Count, callback.Callback, callback.KeepCallback))
                {
                    if (!await callback.Wait(commandKeepCallback))
                    {
                        return AutoCSer.Breakpoint.ReturnFalse();
                ...
                callback = new TwoStageCallback();
                using (var commandKeepCallback = await client.ServerConcurrencyReadQueue_TwoStageCallbackController.Callback(callback.Parameter.Start, callback.Parameter.Count, callback.Callback, callback.KeepCallback))
                {
                    if (!await callback.Wait(commandKeepCallback))
                    {
                        return AutoCSer.Breakpoint.ReturnFalse();
                ...
                callback = new TwoStageCallback();
                using (var commandKeepCallback = await client.ServerReadWriteQueue_TwoStageCallbackController.Callback(callback.Parameter.Start, callback.Parameter.Count, callback.Callback, callback.KeepCallback))
                {
                    if (!await callback.Wait(commandKeepCallback))
                    {
                        return AutoCSer.Breakpoint.ReturnFalse();
                ...
                callback = new TwoStageCallback();
                using (var commandKeepCallback = await client.ServerSynchronous_TwoStageCallbackController.Callback(callback.Parameter.Start, callback.Parameter.Count, callback.Callback, callback.KeepCallback))
                {
                    if (!await callback.Wait(commandKeepCallback))
                    {
                        return AutoCSer.Breakpoint.ReturnFalse();
        ...
        /// <summary>
        /// Keep the callback response test
        /// 持续响应测试
        /// </summary>
