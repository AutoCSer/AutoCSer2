...
namespace AutoCSer.TestCase
{
    class CommandServer
    {
        ...
        /// 创建测试服务端
        /// </summary>
        /// <param name="isShortLink"></param>
        /// <returns></returns>
        internal static CommandListener CreateCommandListener(bool isShortLink)
        {
            ...
            {
                builder = builder.Append<IServerTaskQueueController>(new ServerTaskQueueController())
                    .Append<IServerTaskQueueContextController, int>((task, key) => new ServerTaskQueueContextController(task, key));
            }
            builder = builder.Append<ServerBindContext.IServerSynchronousController>(server => new ServerBindContext.ServerSynchronousController())
