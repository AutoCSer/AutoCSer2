...
{
    internal class ServerJsonBinaryMessageConsumer : LocalServiceServerJsonBinaryMessageConsumer<TestClass>
    {
        internal ServerJsonBinaryMessageConsumer(LocalClient client, IMessageNodeLocalClientNode<ServerJsonBinaryMessage<TestClass>> node) : base(client, node) { }
        ...
        {
            ResponseResult<IMessageNodeLocalClientNode<ServerJsonBinaryMessage<TestClass>>> node = await client.GetOrCreateNode<IMessageNodeLocalClientNode<ServerJsonBinaryMessage<TestClass>>>(typeof(IMessageNodeLocalClientNode<ServerJsonBinaryMessage<TestClass>>).FullName, (index, key, nodeInfo) => client.ClientNode.CreateServerJsonBinaryMessageNode(index, key, nodeInfo, 1 << 10, 5, 1));
            ...
            isCompleted = false;
            using (ServerJsonBinaryMessageConsumer consumer = new ServerJsonBinaryMessageConsumer(client, node.Value))
            {
