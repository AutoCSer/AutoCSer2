...
namespace AutoCSer.TestCase.StreamPersistenceMemoryDatabaseLocalService
{
    internal static class QueueNode
    {
        ...
        {
            ResponseResult<IQueueNodeLocalClientNode<string>> node = await client.GetOrCreateNode<IQueueNodeLocalClientNode<string>, int>(typeof(IQueueNodeLocalClientNode<string>).FullName, 0, client.ClientNode.CreateQueueNode);
