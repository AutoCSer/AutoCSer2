...
{
    internal class ServerJsonMessageConsumer : ServerJsonMessageConsumer<TestClass>
    {
        internal ServerJsonMessageConsumer(CommandClient commandClient, IMessageNodeClientNode<ServerJsonMessage<TestClass>> node) : base(commandClient, node) { }
        ...
        {
            ResponseResult<IMessageNodeClientNode<ServerJsonMessage<TestClass>>> node = await client.GetOrCreateNode<IMessageNodeClientNode<ServerJsonMessage<TestClass>>>(typeof(IMessageNodeClientNode<ServerJsonMessage<TestClass>>).FullName, (index, key, nodeInfo) => client.ClientNode.CreateServerJsonMessageNode(index, key, nodeInfo, 1 << 10, 5, 1));
            ...
            isCompleted = false;
            using (ServerJsonMessageConsumer consumer = new ServerJsonMessageConsumer(commandClient, node.Value))
            {
