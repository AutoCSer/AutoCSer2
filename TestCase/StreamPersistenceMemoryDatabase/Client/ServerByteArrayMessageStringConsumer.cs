...
namespace AutoCSer.TestCase.StreamPersistenceMemoryDatabaseClient
{
    ...
    {
        internal ServerByteArrayMessageStringConsumer(CommandClient commandClient, IMessageNodeClientNode<ServerByteArrayMessage> node) : base(commandClient, node, 1 << 10) { }
        ...
        internal static async Task Test(CommandClient commandClient, AutoCSer.CommandService.StreamPersistenceMemoryDatabaseClient<ICustomServiceNodeClientNode> client)
        {
            ...
            {
                foreach (string message in messages.AutoCSerCollectionExtensions().GetArray())
                {
            ...
            result = await node.Value.AppendMessage((string)null);
            if (!Program.Breakpoint(result)) return;
            completed();
        }
