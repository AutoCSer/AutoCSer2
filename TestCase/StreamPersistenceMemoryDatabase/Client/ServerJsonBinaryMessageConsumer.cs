...
{
    internal class ServerJsonBinaryMessageConsumer : ServerJsonBinaryMessageConsumer<TestClass>
    {
        internal ServerJsonBinaryMessageConsumer(CommandClient commandClient, IMessageNodeClientNode<ServerJsonBinaryMessage<TestClass>> node) : base(commandClient, node) { }
        ...
        {
            ResponseResult<IMessageNodeClientNode<ServerJsonBinaryMessage<TestClass>>> node = await client.GetOrCreateNode<IMessageNodeClientNode<ServerJsonBinaryMessage<TestClass>>>(typeof(IMessageNodeClientNode<ServerJsonBinaryMessage<TestClass>>).FullName, (index, key, nodeInfo) => client.ClientNode.CreateServerJsonBinaryMessageNode(index, key, nodeInfo, 1 << 10, 5, 1));
            ...
            isCompleted = false;
            using (ServerJsonBinaryMessageConsumer consumer = new ServerJsonBinaryMessageConsumer(commandClient, node.Value))
            {
