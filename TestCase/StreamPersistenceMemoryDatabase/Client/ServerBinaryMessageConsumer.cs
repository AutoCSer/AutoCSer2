...
{
    internal class ServerBinaryMessageConsumer : ServerBinaryMessageConsumer<TestClass>
    {
        internal ServerBinaryMessageConsumer(CommandClient commandClient, IMessageNodeClientNode<ServerBinaryMessage<TestClass>> node) : base(commandClient, node) { }
        ...
        {
            ResponseResult<IMessageNodeClientNode<ServerBinaryMessage<TestClass>>> node = await client.GetOrCreateNode<IMessageNodeClientNode<ServerBinaryMessage<TestClass>>>(typeof(IMessageNodeClientNode<ServerBinaryMessage<TestClass>>).FullName, (index, key, nodeInfo) => client.ClientNode.CreateServerBinaryMessageNode(index, key, nodeInfo, 1 << 10, 5, 1));
            ...
            isCompleted = false;
            using (ServerBinaryMessageConsumer consumer = new ServerBinaryMessageConsumer(commandClient, node.Value))
            {
