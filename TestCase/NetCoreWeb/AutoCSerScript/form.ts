...
module AutoCSer {
    ...
    export class Form {
        ...
        constructor(Parameter) {
            Pub.GetParameterEvents(this, Form.DefaultParameter, Form.DefaultEvents, Parameter);
        }
                ...
                if (Value != null) {
                    switch (Form.ElementType(Element)) {
                        ...
                        case 2:
                            ...
                            if (SelectElement.options[0].value == '') SelectElement.selectedIndex = Value as Object as number;
                            else {
        ...
        private KeyDown(Event: BrowserEvent) {
            var Key = (Event.Event as KeyboardEvent).shiftKey || Event.KeyCode, Element = Event.Element as HTMLInputElement, Type = Form.ElementType(Element), Next;
        ...
        private Format(Event: BrowserEvent) {
            var Element = Event.Element as HTMLInputElement, FormElement = this.Elements[Element.name], Type = Form.ElementType(Element);
        ...
        private Focus(Event: BrowserEvent) {
            var Element = Event.Element as HTMLInputElement, FormElement = this.Elements[Element.name];
        ...
        private GetElementValue(Element: HTMLInputElement): any {
            ...
            if (Type == 2) {
                var Value = (Element as Object as HTMLSelectElement).options[(Element as Object as HTMLSelectElement).selectedIndex].value;
