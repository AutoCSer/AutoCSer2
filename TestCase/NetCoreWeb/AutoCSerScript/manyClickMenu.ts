...
module AutoCSer {
    export class ManyClickMenu extends Menu implements IDeclareReset {
        ...
        private Check(Event: DeclareEvent) {
            if (!Event.$ParentName("manyclickmenu")) this.Hide();
        }
        private Show() {
    ...
    Declare.CreateMany(ManyClickMenu, 'ManyClickMenu', 'click', DeclareManyType.ResetParentName);
}