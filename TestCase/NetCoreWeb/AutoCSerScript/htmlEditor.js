...
(function (AutoCSer) {
	var Color512_64 =  (function () {
		function Color512_64(Parameter) {
			AutoCSer.Pub.GetParameterEvents(this, Color512_64.DefaultParameter, Color512_64.DefaultEvents, Parameter);
			this.Identity = AutoCSer.Pub.GetIdentity();
			this.Image = AutoCSer.Pub.JsDomain + 'AutoCSerScript/color512_64.bmp';
		}
		...
		Color512_64.prototype.GetColor = function (Event) {
			var Width = Event.MouseX, Height = Event.MouseY, Color = (((Width >> 5) << 4) + ((Height >> 5) << 3) + 4) << 16;
...
(function (AutoCSer) {
	var CrawlTitle =  (function () {
		...
		CrawlTitle.prototype.OnLink = function (Value) {
			...
			if (Value.Result) {
				this.Link.Title = AutoCSer.Pub.DeleteElements.Html(Value.Result).Text0();
...
(function (AutoCSer) {
	...
	var HtmlEditor =  (function (_super) {
		...
		function HtmlEditor(Parameter) {
			...
			AutoCSer.Pub.GetParameterEvents(_this, HtmlEditor.DefaultParameter, HtmlEditor.DefaultEvents, Parameter);
			_this.OverButtonFunction = AutoCSer.Pub.ThisEvent(_this, _this.OverButton);
			_this.OutButtonFunction = AutoCSer.Pub.ThisEvent(_this, _this.OutButton);
			_this.OnCrawlTitleFunction = AutoCSer.Pub.ThisFunction(_this, _this.OnCrawlTitle);
			_this.ReplaceLinkFunction = AutoCSer.Pub.ThisFunction(_this, _this.ReplaceLink);
			if (!AutoCSer.Pub.IE) {
				_this.PasteImageIdentity = 0;
				if (!HtmlEditor.IsPasteImage)
					_this.PasteImageAjaxCallName = null;
			}
			_this.Identity = AutoCSer.Pub.GetIdentity();
			_this.EditorFrameId = '_' + _this.Identity + 'EDITORIFRAME_';
			_this.SaveFrameId = '_' + _this.Identity + 'EDITORSAVE_';
			_this.TextAreaId = '_' + _this.Identity + 'EDITORINPUT_';
			_this.TempId = '_' + _this.Identity + 'EDITORTEMP_';
			_this.SaveText = '';
			_this.ButtonArray = ['Name,Title,OnlyDesign,OnClick,DefaultSet',
				['FontColor', '字体颜色', 1, AutoCSer.Pub.ThisEvent(_this, _this.SetColor, [true]), 0],
				['BgColor', '字体背景颜色', 1, AutoCSer.Pub.ThisEvent(_this, _this.SetColor, [false]), 0],
				['ClearCode', '清理代码', 0, AutoCSer.Pub.ThisFunction(_this, _this.ClearCode), 1],
				['SelectAll', '全选', 0, AutoCSer.Pub.ThisFunction(_this, _this.SelectAll), 1],
				['Cut', '切剪', 0, AutoCSer.Pub.ThisFunction(_this, _this.Cut), 1],
				['Copy', '复制', 0, AutoCSer.Pub.ThisFunction(_this, _this.Copy), 1],
				['Paste', '粘贴', 0, AutoCSer.Pub.ThisFunction(_this, _this.Paste), 1],
				['Undo', '撤消', 0, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['undo']), 1],
				['Redo', '重做', 0, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['redo']), 1],
				['Unlink', '去掉超级链接', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['Unlink']), 1],
				['InsertHorizontalRule', '插入水平线', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['InsertHorizontalRule']), 1],
				['Bold', '加粗', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['bold']), 1],
				['Italic', '斜体', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['italic']), 1],
				['UnderLine', '下划线', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['underline']), 1],
				['Superscript', '上标', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['superscript']), 1],
				['Subscript', '下标', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['subscript']), 1],
				['StrikeThrough', '删除线', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['strikethrough']), 1],
				['RemoveFormat', '取消格式', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['RemoveFormat']), 1],
				['JustifyLeft', '左对齐', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['justifyleft']), 1],
				['JustifyCenter', '居中', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['justifycenter']), 1],
				['JustifyRight', '右对齐', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['justifyright']), 1],
				['InsertOrderedList', '编号', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['insertorderedlist']), 1],
				['InsertUnOrderedList', '项目符号', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['insertunorderedlist']), 1],
				['OutDent', '减少缩进量', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['outdent']), 1],
				['InDent', '增加缩进量', 1, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['indent']), 1],
				['Quote', '插入引用', 1, AutoCSer.Pub.ThisFunction(_this, _this.Quote), 1],
				['Save', '保存', 0, AutoCSer.Pub.ThisFunction(_this, _this.Save), 1],
				['LoadSave', '加载', 0, AutoCSer.Pub.ThisFunction(_this, _this.LoadSave), 1],
				['ClearAll', '清除', 0, AutoCSer.Pub.ThisFunction(_this, _this.ClearAll), 1],
				['Print', '打印', 0, AutoCSer.Pub.ThisEvent(_this, _this.ExecCommand, ['print']), 1],
				['FormatCode', '整理代码', 0, AutoCSer.Pub.ThisFunction(_this, _this.FormatCode), 1]
			].FormatAjax();
			_this.ChangeParagraphFunction = AutoCSer.Pub.ThisFunction(_this, _this.ChangeParagraph);
			_this.ChangeFontNameFunction = AutoCSer.Pub.ThisFunction(_this, _this.ChangeFontName);
			_this.ChangeFontSizeFunction = AutoCSer.Pub.ThisFunction(_this, _this.ChangeFontSize);
			_this.SetColorFunction = AutoCSer.Pub.ThisEvent(_this, _this.SetColor);
			_this.OverColorFunction = AutoCSer.Pub.ThisEvent(_this, _this.OverColor);
			_this.SetDesignModeFunction = AutoCSer.Pub.ThisFunction(_this, _this.SetMode, [true]);
			_this.SetInputModeFunction = AutoCSer.Pub.ThisFunction(_this, _this.SetMode, [false]);
		...
		HtmlEditor.prototype.OverButton = function (Event) {
			if (this.ButtonOverClassName)
				Event.Element.className = this.ButtonOverClassName;
		};
		HtmlEditor.prototype.OutButton = function (Event) {
			if (this.ButtonOutClassName)
				Event.Element.className = this.ButtonOutClassName;
		};
		...
		HtmlEditor.prototype.PasteFilter = function (Event) {
			if (this.PasteLinkAjaxCallName || this.PasteImageAjaxCallName || this.OnPasteFilter.Get().length) {
				...
				if (AutoCSer.Pub.IE) {
					...
					SaveFrame.focus();
					SaveFrame.document.execCommand('selectAll', false, null);
					SaveFrame.document.execCommand('paste', false, null);
		...
		HtmlEditor.prototype.PasteFilterEnd = function () {
			...
			if (Divs.length) {
				...
				if (this.PasteImageFormData) {
					this.PasteImageFormData.append('identity', this.PasteImageIdentity.toString());
		...
		HtmlEditor.prototype.FormatCode = function () {
			...
			for (var Spans = new AutoCSer.HtmlElement("/span", Body).GetElements(), Index = Spans.length; Index; AutoCSer.View.DeleteMark(Spans[--Index]))
				;
		...
		HtmlEditor.prototype.ChangeFontSize = function () {
			...
			frames[this.EditorFrameId].document.execCommand('fontsize', false, HtmlEditor.FontSizes[FontSize.selectedIndex - 1]);
		...
		HtmlEditor.prototype.StartButton = function () {
			...
			if (FontSize.Element0()) {
				if (!HtmlEditor.FontSizeOptionHtml) {
					HtmlEditor.FontSizes = [];
					for (var Html = [], Index = 1; Index <= 7; Index++) {
						var Size = (Index * 7) + 'px';
						HtmlEditor.FontSizes.push(Size);
						Html.push("<option style='font-size:" + Size + "'>" + Index + '</option>');
					}
	...
	AutoCSer.Declare.Create(HtmlEditor, 'HtmlEditor', 'click', AutoCSer.DeclareType.EventElement);
