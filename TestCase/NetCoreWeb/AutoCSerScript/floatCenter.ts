...
module AutoCSer {
    export class FloatCenter {
        private static DefaultParameter = { Id: null, IsFixed: false, IsEsc: true, Timeout: 200, ViewData: null, ZIndex: 0, AutoClass: null };
        private Id: string;
        ...
        constructor(Parameter) {
            ...
            if (this.ViewData) this.View = View.Views[this.Id] || new View(this.Id);
            if (Pub.IsFixed) {
        ...
        Show() {
            ...
            if (Pub.IsFixed) {
                ...
                if (this.AutoClass) {
                    if (this.View) this.View.SetHtml(this.ViewData);
                    Element.AddClass(this.AutoClass);
                }
                else {
                    if (this.View) this.View.Show(this.ViewData);
        ...
        private KeyDown(Event: BrowserEvent) {
            if (Event.KeyCode == 27) {
                var Element = Event.$ParentName('floatcenter');
