...
'use strict';
AutoCSer.Pub.SendError = function (Error: AutoCSer.ISendError) {
...
window.onerror = function (message: string, filename?: string, lineno?: number, colno?: number, error?: Error): void {
    ...
    var SendError = { Message: message } as AutoCSer.ISendError, Location = document.location.toString();
    if (filename && Location.substring(0, filename.length) != filename) SendError.FileName = filename;
    if (lineno) SendError.LineNo = lineno;
    if (colno) SendError.ColNo = colno;
