...
namespace AutoCSer.TestCase.SearchQueryService
{
    ...
    internal sealed class SearchUserNode : ConditionDataLocalNode<ISearchUserNode, int, SearchUser, ISearchUserNodeLocalClientNode>, ISearchUserNode, IEnumerableSnapshot<bool>
    {
        ...
        /// <returns></returns>
        protected override LocalServiceQueueNode<LocalResult> loadCreate(ISearchUserNodeLocalClientNode client, SearchUser value)
        {
        ...
        public ArrayBuffer<int> Filter(ArrayBuffer<int> userIds, Func<SearchUser, bool> isValue)
        {
            ...
            {
                if (users.TryGetValue(userId, out user) && isValue(user)) userIds.Add(userId);
            }
            return userIds;
        }
