...
namespace AutoCSer.TestCase.SearchQueryService
{
    ...
    internal sealed class LocalDiskBlockIndexServiceConfig : LocalServiceConfig
    {
        ...
        /// 用户名称索引节点单例
        /// </summary>
        public static readonly AutoCSer.CommandService.StreamPersistenceMemoryDatabaseLocalClientNodeCache<IRemoveMarkHashKeyIndexNodeLocalClientNode<int>> UserNameNodeCache = Client.CreateNode(client => client.GetOrCreateNode<IRemoveMarkHashKeyIndexNodeLocalClientNode<int>>(nameof(OperationDataTypeEnum.UserNameNode), (index, key, nodeInfo) => client.ClientNode.CreateRemoveMarkHashKeyIndexNode(index, key, nodeInfo, typeof(int), 0)));
        /// <summary>
        /// 用户备注索引节点单例
        /// </summary>
        public static readonly AutoCSer.CommandService.StreamPersistenceMemoryDatabaseLocalClientNodeCache<IRemoveMarkHashKeyIndexNodeLocalClientNode<int>> UserRemarkNodeCache = Client.CreateNode(client => client.GetOrCreateNode<IRemoveMarkHashKeyIndexNodeLocalClientNode<int>>(nameof(OperationDataTypeEnum.UserRemarkNode), (index, key, nodeInfo) => client.ClientNode.CreateRemoveMarkHashKeyIndexNode(index, key, nodeInfo, typeof(int), 0)));
    }
}
