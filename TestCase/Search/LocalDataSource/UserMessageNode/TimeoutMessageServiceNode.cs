...
namespace AutoCSer.TestCase.SearchDataSource
{
    ...
    /// </summary>
    public sealed class TimeoutMessageServiceNode : AutoCSer.CommandService.StreamPersistenceMemoryDatabase.ServiceNode, ITimeoutMessageServiceNode
    {
        /// <summary>
        /// 用户搜索数据更新消息节点的自定义基础服务
        /// </summary>
        /// <param name="service">日志流持久化内存数据库服务端</param>
        public TimeoutMessageServiceNode(StreamPersistenceMemoryDatabaseService service) : base(service) { }
        /// <summary>
        ...
        {
            return CreateSnapshotNode<ITimeoutMessageNode<OperationData<int>>>(index, key, nodeInfo, () => new LocalSearchUserMessageNode(timeoutSeconds));
        }
    }
}
