...
namespace AutoCSer.TestCase.SearchQueryService
{
    ...
    /// 分词结果磁盘块索引信息节点服务端配置
    /// </summary>
    internal sealed class MemoryWordIdentityBlockIndexServiceConfig : LocalServiceConfig
    {
        /// <summary>
        ...
        /// 日志流持久化内存数据库本地服务端
        /// </summary>
        private static readonly LocalService localService = new MemoryWordIdentityBlockIndexServiceConfig
        {
            PersistencePath = Path.Combine(AutoCSer.TestCase.Common.Config.AutoCSerTemporaryFilePath, nameof(MemoryWordIdentityBlockIndexServiceConfig)),
            PersistenceSwitchPath = Path.Combine(AutoCSer.TestCase.Common.Config.AutoCSerTemporaryFilePath, nameof(MemoryWordIdentityBlockIndexServiceConfig) + nameof(PersistenceSwitchPath))
