...
namespace AutoCSer.TestCase.SearchQueryService
{
    ...
    public sealed class QueryServiceNode : AutoCSer.CommandService.StreamPersistenceMemoryDatabase.ServiceNode, IQueryServiceNode
    {
        ...
        {
            return CreateSnapshotNode<ISearchUserNode>(index, key, nodeInfo, () => new SearchUserNode(Service));
        }
        /// <summary>
        ...
        /// <returns>Node identifier, there have been a node is returned directly
        /// 节点标识，已经存在节点则直接返回</returns>
        public NodeIndex CreateHashIndexNode(NodeIndex index, string key, NodeInfo nodeInfo, AutoCSer.Reflection.RemoteType keyType, AutoCSer.Reflection.RemoteType valueType)
        {
            ...
            {
                return (NodeIndex)typeof(QueryServiceNode).GetMethod(nameof(createHashIndexNode), BindingFlags.Static | BindingFlags.NonPublic)
        ...
        /// <summary>
        /// 创建带移除标记的可重用哈希索引节点 IHashIndexNode{KT,VT}
        /// </summary>
        ...
        /// <returns>Node identifier, there have been a node is returned directly
        /// 节点标识，已经存在节点则直接返回</returns>
        private static NodeIndex createHashIndexNode<KT, VT>(ServiceNode node, NodeIndex index, string key, NodeInfo nodeInfo)
            where KT : notnull, IEquatable<KT>
            where VT : notnull, IEquatable<VT>
        {
            return node.CreateSnapshotNode<IHashIndexNode<KT, VT>>(index, key, nodeInfo, () => new HashIndexNode<KT, VT>());
        }
        /// <summary>
        ...
        public NodeIndex CreateHashCodeKeyIndexNode(NodeIndex index, string key, NodeInfo nodeInfo, AutoCSer.Reflection.RemoteType keyType)
        {
            var type = default(Type);
            CallStateEnum state = getEquatableType(ref keyType, ref type);
            if (state == CallStateEnum.Success)
            {
                return (NodeIndex)typeof(QueryServiceNode).GetMethod(nameof(createHashCodeKeyIndexNode), BindingFlags.Static | BindingFlags.NonPublic)
        ...
        /// <summary>
        /// 创建带移除标记的可重用哈希索引节点 IHashCodeKeyIndexNode{T}
        /// </summary>
        ...
        /// <returns>Node identifier, there have been a node is returned directly
        /// 节点标识，已经存在节点则直接返回</returns>
        private static NodeIndex createHashCodeKeyIndexNode<T>(ServiceNode node, NodeIndex index, string key, NodeInfo nodeInfo)
            where T : notnull, IEquatable<T>
        {
            return node.CreateSnapshotNode<IHashCodeKeyIndexNode<T>>(index, key, nodeInfo, () => new HashCodeKeyIndexNode<T>());
        }
    }
}
