        ...
        public static Config Get(string JsonFileName = null)
        {
            if (!string.IsNullOrEmpty(JsonFileName))
            {
                ...
                if (JsonFile.Exists)
                {
                    ...
                    AutoCSer.LogHelper.Error("JSON 配置文件 " + JsonFile.FullName + " 解析失败", LogLevelEnum.Error | LogLevelEnum.AutoCSer).AutoCSerNotWait();
                }
                else AutoCSer.LogHelper.Error("没有找到 JSON 配置文件 " + JsonFile.FullName, LogLevelEnum.Error | LogLevelEnum.AutoCSer).AutoCSerNotWait();
            }
            return Default;
        }
        /// <summary>
