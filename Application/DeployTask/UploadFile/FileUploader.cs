...
namespace AutoCSer.CommandService.DeployTask
{
    ...
    internal sealed class FileUploader : SecondTimerTaskArrayNode
    {
        ...
        /// 上传完成文件集合
        /// </summary>
        internal LeftArray<KeyValue<FileInfo, FileInfo>> UploadCompletedFiles;
        /// <summary>
        /// 等待删除的文件集合
        /// </summary>
        internal LeftArray<KeyValue<FileInfo, FileInfo>> DeleteFiles;
        /// <summary>
        /// 等待删除的目录集合
        /// </summary>
        internal LeftArray<KeyValue<DirectoryInfo, string>> DeleteDirectorys;
        /// <summary>
        /// 文件上传
        /// </summary>
        ...
        /// <param name="extensions">扩展名称集合</param>
        /// <param name="isCaseExtension">扩展名匹配是否区分大小写</param>
        internal FileUploader(UploadFileService service, DirectoryInfo directory, DirectoryInfo backupDirectory, string[] extensions, bool isCaseExtension)
            : base(SecondTimer.TaskArray, service.TimeoutSeconds, SecondTimerTaskThreadModeEnum.Synchronous, SecondTimerKeepModeEnum.After, service.TimeoutSeconds)
        {
            ...
            if (isCaseExtension) this.extensions = new HashSet<string>(extensions);
            else
            {
        ...
        internal async Task<bool> Completed()
        {
            ...
            while (uploadCompletedFileCount != 0)
            {
                ...
                {
                    string fileName = uploadFile.Key.FullName, backupFileName = uploadFile.Value.FullName, movBackupFileName = backupFileName + AutoCSer.Threading.SecondTimer.Now.ToString(".yyyyMMddHHmmss") + ".bak";
                    await AutoCSer.Common.FileMove(uploadFile.Key, movBackupFileName);
                    await AutoCSer.Common.FileMove(uploadFile.Value, fileName);
                    await AutoCSer.Common.FileMove(movBackupFileName, backupFileName);
                }
                else await AutoCSer.Common.FileMove(uploadFile.Value, uploadFile.Key.FullName);
            }
        ...
        /// </summary>
        /// <returns></returns>
        internal async Task<string> GetUploadCompletedFile()
        {
            uploadTimestamp = Stopwatch.GetTimestamp();
            SubArray<byte> compressData;
            Monitor.Enter(uploadLock);
            for (int index = fileIndex; index != 0;)
            {
                if (files[--index].FileStream != null)
            ...
            try
            {
                compressData = getUploadCompletedFileCompressData();
            }
            finally { Monitor.Exit(uploadLock); }

            uploadTimestamp = Stopwatch.GetTimestamp();
            string fileName = UploaderInfo.BackupPath + ".ucf";
#if NetStandard21
...
#endif
            {
                await fileStream.WriteAsync(compressData.Array, 0, compressData.Length + sizeof(int));
            }
        ...
        /// </summary>
        /// <returns></returns>
        private unsafe SubArray<byte> getUploadCompletedFileCompressData()
        {
            byte[] data = AutoCSer.BinarySerializer.Serialize(new UploadCompletedFiles(this));
            SubArray<byte> compressData = AutoCSer.Common.Config.Compress(data, 0, data.Length, sizeof(int), System.IO.Compression.CompressionLevel.Fastest);
            fixed (byte* dataFixed = compressData.GetFixedBuffer()) *(int*)dataFixed = data.Length;
            return compressData;
        }
    }
}
