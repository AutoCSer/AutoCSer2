...
namespace AutoCSer.CommandService.Search.RemoveMarkHashIndexCache
{
    ...
    public abstract class GenericKeyUIntValueCache<KT, VT, NT> : GenericKeyCache<KT, VT, NT>
#if NetStandard21
        ...
        where NT : UIntIndex<KT, VT>
    {
        ...
        /// 带移除标记的可重用哈希索引节点接口
        /// </summary>
        protected readonly StreamPersistenceMemoryDatabaseClientNodeCache<IRemoveMarkHashKeyIndexNodeClientNode<KT>> node;
        /// <summary>
        /// 索引数据磁盘块索引缓存
        /// </summary>
        ...
        protected override async Task getChangeKeys()
        {
            ...
            do
            {
                ResponseResult<IRemoveMarkHashKeyIndexNodeClientNode<KT>> nodeResult = await node.GetSynchronousNode();
    ...
    /// </summary>
    /// <typeparam name="KT">索引关键字类型</typeparam>
    public abstract class GenericKeyUIntValueCache<KT> : GenericKeyUIntValueCache<KT, uint, UIntIndex<KT>>
#if NetStandard21
        where KT : notnull, IEquatable<KT>
#else
        where KT : IEquatable<KT>
#endif
    {
        ...
        /// <param name="maxCount">最大缓存数据数量，少量数据为 8B 元素，大量数据为 16B 元素</param>
        /// <param name="capacity">容器初始化大小</param>
        protected GenericKeyUIntValueCache(StreamPersistenceMemoryDatabaseClientNodeCache<IRemoveMarkHashKeyIndexNodeClientNode<KT>> node, long maxCount, int capacity = 1 << 16) : base(node, maxCount, capacity)
        {
        }
        /// <summary>
        /// 创建索引数据磁盘块索引缓存节点
