...
namespace AutoCSer.CommandService.Search.RemoveMarkHashIndexCache
{
    ...
    public abstract class UIntKeyLocalCache<VT> : UIntKeyCache<VT, GenericIndex<uint, VT>>
#if NetStandard21
...
#endif
    {
        ...
        /// 带移除标记的可重用哈希索引节点接口
        /// </summary>
        private readonly StreamPersistenceMemoryDatabaseLocalClientNodeCache<IRemoveMarkHashIndexNodeLocalClientNode<uint, VT>> node;
        /// <summary>
        /// 索引数据磁盘块索引缓存 uint:VT
        /// </summary>
        ...
        protected UIntKeyLocalCache(StreamPersistenceMemoryDatabaseLocalClientNodeCache<IRemoveMarkHashIndexNodeLocalClientNode<uint, VT>> node, long maxCount, int capacity = 1 << 16) : base(maxCount, capacity)
        {
            this.node = node;
        ...
        protected override async Task getChangeKeys()
        {
            ...
            do
            {
                LocalResult<IRemoveMarkHashIndexNodeLocalClientNode<uint, VT>> nodeResult = await node.GetSynchronousNode();
        ...
        {
            LocalResult<IRemoveMarkHashIndexNodeLocalClientNode<uint, VT>> nodeResult = await this.node.GetNode();
