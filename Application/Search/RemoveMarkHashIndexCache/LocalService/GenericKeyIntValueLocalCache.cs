...
namespace AutoCSer.CommandService.Search.RemoveMarkHashIndexCache
{
    ...
    public abstract class GenericKeyIntValueLocalCache<KT, NT> : GenericKeyUIntValueLocalCache<KT, int, IntIndex<KT>>
#if NetStandard21
...
#endif
    {
        ...
        /// 容器初始化大小</param>
        protected GenericKeyIntValueLocalCache(StreamPersistenceMemoryDatabaseLocalClientNodeCache<IRemoveMarkHashKeyIndexNodeLocalClientNode<KT>> node, long maxCount, int capacity = 1 << 16) : base(node, maxCount, capacity)
        {
        }
        /// <summary>
        /// 创建索引数据磁盘块索引缓存节点
        ...
        internal override async Task<ResponseResult> GetBlockIndexData(BlockIndexDataCacheNode<KT, int> node)
        {
            ...
            do
            {
                ...
                LocalResult<BlockIndexData<int>> data = await nodeResult.Value.notNull().GetIntBlockIndexData(node.Key);
                if (data.IsSuccess)
                {
