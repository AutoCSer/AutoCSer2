...
namespace AutoCSer.CommandService.Search.WordIdentityBlockIndex
{
    ...
    public abstract class LocalNode<T> : MethodParameterCreatorNode<ILocalNode<T>, BinarySerializeKeyValue<T, BlockIndex>>, ILocalNode<T>, ISnapshot<BinarySerializeKeyValue<T, BlockIndex>>, ISnapshot<bool>
#if NetStandard21
...
#endif
    {
        ...
        /// 分词数据磁盘块索引信息集合
        /// </summary>
        internal readonly FragmentDictionary256<T, LocalData<T>> Datas;
        /// <summary>
        /// 操作队列访问锁集合
        /// </summary>
        ...
        /// </summary>
        protected readonly StreamPersistenceMemoryDatabaseLocalClientNodeCache<ILocalNodeLocalClientNode<T>> loadClientNode;
        /// <summary>
        /// 获取字符串 Trie 图节点单例
        /// </summary>
        protected readonly StreamPersistenceMemoryDatabaseLocalClientNodeCache<IStaticTrieGraphNodeLocalClientNode> trieGraphNode;
        /// <summary>
        ...
        protected LocalNode(StreamPersistenceMemoryDatabaseLocalClientNodeCache<IStaticTrieGraphNodeLocalClientNode> trieGraphNode, StreamPersistenceMemoryDatabaseLocalClientNodeCache<ILocalNodeLocalClientNode<T>> loadClientNode) 
        {
            this.trieGraphNode = trieGraphNode;
            this.loadClientNode = loadClientNode;
            Datas = new FragmentDictionary256<T, LocalData<T>>();
...
#endif
        {
            foreach (KeyValuePair<T, LocalData<T>> data in Datas.KeyValues) data.Value.Loaded(this, data.Key);
        ...
        protected virtual async Task load()
        {
            ...
            do
            {
                try
                {
                    LocalResult<ILocalNodeLocalClientNode<T>> nodeResult = await loadClientNode.GetSynchronousNode();
                    if (nodeResult.IsSuccess)
                    {
                        ...
                        if (command != null)
                        {
                            ILocalNodeLocalClientNode<T> node = nodeResult.Value.notNull();
                            BinarySerializeKeyValue<T, string>[] values = new BinarySerializeKeyValue<T, string>[Math.Max(loadArraySize, 1)];
                            LocalLoadCallback<T> callback = new LocalLoadCallback<T>(this, values);
                            int index = 0;
        ...
        private async Task<int> create(ILocalNodeLocalClientNode<T> node, LocalLoadCallback<T> callback)
        {
            ...
            {
                LocalResult<WordIdentityBlockIndexUpdateStateEnum> state = await responses[--index];
        ...
        SnapshotResult<BinarySerializeKeyValue<T, BlockIndex>> ISnapshot<BinarySerializeKeyValue<T, BlockIndex>>.GetSnapshotResult(BinarySerializeKeyValue<T, BlockIndex>[] snapshotArray, object customObject)
        {
            ...
            foreach (KeyValuePair<T, LocalData<T>> data in Datas.KeyValues) result.Add(snapshotArray, new BinarySerializeKeyValue<T, BlockIndex>(data.Key, data.Value.BlockIndex));
            return result;
        }
        /// <summary>
        /// 快照设置数据
        ...
        public void LoadCreate(T key, string text, MethodCallback<WordIdentityBlockIndexUpdateStateEnum> callback)
        {
            ...
            try
            {
                ...
                {
                    LocalData<T> data = new LocalData<T>();
                    Datas.Add(key, data);
        ...
        public void CreateLoadPersistence(T key, MethodCallback<WordIdentityBlockIndexUpdateStateEnum> callback)
        {
            if (key != null)
            {
                var data = default(LocalData<T>);
                if (Datas.TryGetValue(key, out data)) data.IsLoadedDeleted = false;
                else Datas.Add(key, new LocalData<T>());
            }
        }
        /// <summary>
        /// 创建分词结果磁盘块索引信息
        ...
        public void Update(T key, MethodCallback<WordIdentityBlockIndexUpdateStateEnum> callback)
        {
            ...
            try
            {
                if (key != null)
                {
                    ...
                    {
                        Datas.Add(key, data = new LocalData<T>());
                        data.Create(this, key, callback).NotWait();
                    }
                    state = WordIdentityBlockIndexUpdateStateEnum.Callbacked;
                }
        ...
        /// <returns></returns>
        public virtual Task OnException(LocalData<T> data, Exception exception) { return AutoCSer.Common.CompletedTask; }
        /// <summary>
        /// 根据关键字获取需要分词的文本数据
        /// </summary>
        /// <param name="key">分词数据关键字</param>
        ...
        public virtual async Task<LocalResult<int[]>> GetWordIdentitys(string text)
        {
            LocalResult<IStaticTrieGraphNodeLocalClientNode> node = await trieGraphNode.GetSynchronousNode();
        ...
        /// <returns></returns>
        public abstract Task<LocalResult> AppendIndex(int[] wordIdentitys, T key);
        /// <summary>
        /// 更新分词匹配数据关键字
        /// </summary>
        /// <param name="wordIdentitys">词语编号标识集合</param>
        ...
        /// <param name="key">匹配数据关键字</param>
        /// <returns></returns>
        public abstract Task<LocalResult> AppendIndex(int[] wordIdentitys, int[] historyWordIdentitys, T key);
        /// <summary>
        /// 删除分词匹配数据关键字
        /// </summary>
        /// <param name="wordIdentitys">词语编号标识集合</param>
        /// <param name="key">匹配数据关键字</param>
        /// <returns></returns>
        public abstract Task<LocalResult> RemoveIndex(int[] wordIdentitys, T key);
    }
}
