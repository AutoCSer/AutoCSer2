...
namespace AutoCSer.CommandService.StreamPersistenceMemoryDatabase
{
    ...
    /// 客户端节点接口类型</typeparam>
    public abstract class LocalClientReturnValueNode<T> : IDisposable
        where T : class
    {
        /// <summary>
        /// Client node
        ...
        public LocalClientReturnValueNode(StreamPersistenceMemoryDatabaseLocalClientNodeCache<T> node, bool isIgnoreError = false, bool isSynchronousCallback = false)
        {
        ...
        /// <returns></returns>
        private async System.Threading.Tasks.Task getNode(StreamPersistenceMemoryDatabaseLocalClientNodeCache<T> node, bool isSynchronousCallback)
        {
            try
            {
        ...
        /// 是否结束程序</returns>
        private bool getNode(StreamPersistenceMemoryDatabaseLocalClientNodeCache<T> node, LocalResult<T> result)
        {
            if (result.IsSuccess)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public LocalResult<T> GetNodeResult()
        {
            if (node != null) return node;
            return new LocalResult<T>(callState, exception);
        }
        /// <summary>
        /// Get the client node
        /// 获取客户端节点
        /// </summary>
        /// <returns></returns>
        public Task<LocalResult<T>> GetNodeResultAsync()
        {
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<LocalResult<T>> getNodeResultAsync()
        {
        ...
        protected LocalServiceReturnValue getReturnValue(LocalServiceQueueNode<LocalResult> node)
        {
            return new LocalServiceReturnValue(node, isIgnoreError);
        }
        /// <summary>
        /// 直接获取返回值
        /// </summary>
        ...
        /// <returns></returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected LocalServiceReturnValue<VT> getReturnValue<VT>(LocalServiceQueueNode<LocalResult<VT>> node)
        {
            return new LocalServiceReturnValue<VT>(node, isIgnoreError);
        }
        /// <summary>
        /// 代码生成模板
        /// </summary>
        ...
        /// <returns></returns>
        internal int getReturnValue(object value) { return 0; }
    }
}
